...
. Add this https://raw.githubusercontent.com/honnuanand/pivotal-Workshop/master/labs/springboot/import.sql[import.sql] file found in *pivotal-workshop/labs/springboot/* to +src/main/resources+. This is a rather large dataset containing all of the postal codes in the United States and its territories. This file will automatically be picked up by http://hibernate.org/orm/what-is-an-orm/[Hibernate] and imported into the in-memory database.
